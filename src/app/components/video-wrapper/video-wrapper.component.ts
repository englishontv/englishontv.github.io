...
export class VideoWrapperComponent implements OnInit {
  ...
  private videoListeners: any = {
    loadedmetadata: () => this.videoTimeService.setVideoDuration(this.video != null ? this.video.nativeElement.duration : 0),
    canplay: () => this.videoService.setLoading(false),
    seeking: () => this.videoService.setLoading(true),
    timeupdate: () => {
      if (!this.ignore && this.video != null) {
  ...
  public onDoubleClick() {
    if (document.fullscreenElement) {
      ...
      if (videoPlayerDiv != null && videoPlayerDiv.requestFullscreen) {
  ...
  public load(currentVideo: string): void {
    if (HLS.isSupported()) {
      ...
      if (this.video != null && this.video.nativeElement.canPlayType('application/vnd.apple.mpegurl')) {
  ...
  private subscriptions() {
    ...
    this.videoTimeService.currentTime$.subscribe(currentTime => (this.video != null ? this.video.nativeElement.currentTime = currentTime : 0));
    this.volumeService.volumeValue$.subscribe(volume => (this.video != null ? this.video.nativeElement.volume = volume :0));
